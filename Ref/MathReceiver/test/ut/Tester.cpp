...
namespace Ref {
    ...
    testDivZero()
  {
      ...
      this->invoke_to_mathOpIn(0, val1, MathOp::DIV, val2);
      ...
      // check that the component handled the invalid operation correctly
      ASSERT_from_mathOpReject(0, MathErr(MathErr::DivByZero), val1, MathOp(MathOp::DIV), val2);
  ...
  void Tester ::
    from_mathOpReject_handler(
        const NATIVE_INT_TYPE portNum,
        const Ref::MathErr &err,
        F32 val1,
        const Ref::MathOp &op,
        F32 val2
    )
  {
    this->pushFromPortEntry_mathOpReject(err, val1, op, val2);
  }
    ...
    connectPorts()
  {
    ...
    // mathOpReject
    this->component.set_mathOpReject_OutputPort(
        0,
        this->get_from_mathOpReject(0)
    );
